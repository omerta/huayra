...
class sigesp_sno_c_constantes
{
	...
	function sigesp_sno_c_constantes()
	{
		...
		// Fecha Creaci�n: 15/02/2006 								Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_select_valor($as_codcons)
	{
		...
		// Modificado Por: Ing. Yesenia Moreno						Fecha �ltima Modificaci�n : 30/05/2006
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $ls_total=0;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_select_valor ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_check_seguridad($as_codcons)
	{
		...
		//	    Arguments: as_codcons // C�digo de la Constante
		//	      Returns: lb_valido -> variable boolean
		//	  Description: verifica si la persona tiene acceso a dicha constante
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 05/11/2007
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_check_seguridad ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_select_constantes($as_codcons)
	{
		...
		{
			$lb_existe=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_select_constantes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if(!$row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_insert_constantes($as_codcons,$as_nomcon,$as_unicon,$as_topcon,$as_valcon,$as_reicon,$as_conespseg,$as_esttopmod,$as_perenc,$aa_seguridad)
	{
		...
		//				   as_conespseg   // constante especial en n�mina
		//                 as_esttopmod     // tope modificable por persona
		//                 as_perenc // indica si la constante pertenece a encargadur�a
		//	    		   aa_seguridad // arreglo de la variable de seguridad
		//	      Returns: lb_valido -> variable boolean
		//	  Description: Guarda los datos de la constante
		//	   Creado Por: Ing. Yozelin Barragan
		// Fecha Creaci�n: 15/02/2006
		// Modificado Por: Ing. Yesenia Moreno						Fecha �ltima Modificaci�n : 01/10/2007
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_equcon="1";
		$ls_tipnumcon="1";
		...
		if($li_row===false)
		{
			...
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_insert_constantes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la constante ".$as_codcons." asociado a la n�mina ".$this->ls_codnom;
	...
	function uf_insert_const_personal($as_codcons,$as_moncon,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_insert_const_personal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la constantepersonal constante ".$as_codcons."  asociado a la n�mina ".$this->ls_codnom;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_update_constantes($as_codcons,$as_nomcon,$as_unicon,$as_topcon,$as_valcon,$as_reicon,$as_conespseg,$as_esttopmod,$as_perenc,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			...
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_update_constantes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualiz� la constante ".$as_codcons." asociado a la n�mina ".$this->ls_codnom;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
	                    ...
	                    $as_esttopmod,$as_perenc,$aa_seguridad)
	{
		...
		//	    Arguments: as_codcons  // c�digo de la constante						as_nomcon  // Nombre
	...
	function uf_delete_constantepersonal($as_codcons)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_delete_constantepersonal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		return $lb_valido;
    ...
    function uf_delete_constante($as_codcons,$aa_seguridad)
    {
		...
		//		   Access: public (sigesp_sno_d_constante)
		//	    Arguments: as_codcons  // c�digo de constante
		//				   aa_seguridad  // arreglo de las variables de seguridad
		...
		if($lb_valido)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->Constante M�TODO->uf_delete_constante ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
				$this->io_sql->rollback();
			}
			else
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� la constante  ".$as_codcons." asociada a la n�mina ".$this->ls_codnom;
				$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_load_constantepersonal($as_codcons,$ai_inicio,$ai_registros,&$ai_totrows,&$ao_object,&$ai_totpag,$as_esttopmod)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Concepto M�TODO->uf_load_constantepersonal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_obtener_constantepersonal($as_codper,$as_codcons,&$ai_valor)
	{
		...
		//				   as_codcons // c�digo de la constante
		...
		//				   esta funci�n se llama desde la clase evaluadora
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creaci�n: 15/02/2006
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
	...
	function update_const_personal($as_codcons,$as_moncon,$as_topcon,$as_codper)
	{
		...
		if($li_row===false){
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->update_const_personal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
	                           ...
	                           &$as_esttopmod,&$as_perenc)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Concepto M�TODO->uf_load_constante ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_aplicar_valor($as_codcons,$ai_moncon,$ai_topcon,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_aplicar_valor ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="actualiz� la constantepersonal constante ".$as_codcons." asociado a la n�mina ".$this->ls_codnom;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_load_constantes_x_personal($as_codper,&$ai_totrows,&$aa_object)
	{
		...
		//	    Arguments: as_codper  // C�digo de persona�
		...
		//	  Description: Funci�n que obtiene el sueldo de un personal dado un � sueldo
		//	   Creado Por: Ing. Yesenia Moreno
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_load_constantes_x_personal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_update_constantes_x_personal($as_codcons,$as_codper,$ai_moncon,$ai_topcon,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_update_constantes_x_personal ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualiz� la constante ".$as_codcons." del personal ".$as_codper." asociado a la n�mina ".$this->ls_codnom;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
    ...
    function uf_load_constantesencargaduria(&$ai_totrows,&$ao_object)
    {
		...
		//////////////////////////////////////////////////////////////////////////////
 		$lb_valido=true;
       	...
       	if ($rs_data===false)
       	{
        	$this->io_mensajes->message("CLASE->Constante N�mina M�TODO->uf_load_constantesencargaduria ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
       	}
       	else
       	{
	...
	function uf_guardar_constantesencargaduria($as_codconc,$as_perenc,$aa_seguridad)
	{
		...
		//				   as_titcon  // T�tulo
		//	      Returns: lb_valido True si se ejecuto el update � False si hubo error en el update
		//	  Description: Funcion que actualiza la constante seleccionada por encargadur�a
		//	   Creado Por: Ing. Mar�a Beatriz Unda
		// Fecha Creaci�n: 29/12/2008 								Fecha �ltima Modificaci�n : 		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="UPDATE sno_constante ".
		...
		{
 			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Constante M�TODO->uf_guardar_constantesencargaduria ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
