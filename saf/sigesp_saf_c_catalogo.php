...
class sigesp_saf_c_catalogo
{
	...
	function uf_saf_select_catalogo($as_catalogo)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->catalogo M�TODO->uf_saf_select_catalogo ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_saf_insert_catalogo($as_catalogo,$as_denominacion,$as_cuenta,$aa_seguridad)
	{
		...
		{
			$lb_valido[0]=false;
			$lb_valido[1]=$this->io_msg->message;
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="INSERT";
				$ls_descripcion ="Insert&oacute; en el Catalogo SIGECOF ".$as_catalogo;
	...
	function uf_saf_update_catalogo($as_catalogo,$as_denominacion,$as_cuenta,$aa_seguridad)
	{
		 ...
		 $ls_sql = "UPDATE saf_catalogo SET   dencat='". $as_denominacion ."', spg_cuenta='". $as_cuenta ."' ".
			...
			else
			{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="UPDATE";
				$ls_descripcion ="Actualiz&oacute; ".$as_catalogo." en el Catalogo SIGECOF ";
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				if($ls_variable[0] == false)
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
					$ls_evento="DELETE";
					$ls_descripcion ="Elimin&oacute; el Catalogo SIGECOF ".$as_catalogo;
					$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					if($ls_variable[0] == false)
	...
	function uf_saf_select_activos($as_catalogo)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->catalogo M�TODO->uf_saf_select_activos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_saf_select_articulos($as_catalogo)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->catalogo M�TODO->uf_saf_select_articulos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
