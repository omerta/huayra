...
class sigesp_saf_c_unidadfisica
{
	...
	function uf_saf_select_unidadfisica($as_coduniadm)
	{
		...
		// Fecha Creaci�n: 21/11/2007 								Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->unidadfisica M�TODO->uf_saf_select_unidadfisica ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_saf_insert_unidadfisica($as_coduniadm,$as_denuniadm,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->unidadfisica M�TODO->uf_saf_insert_cambioresponsable ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="INSERT";
			$ls_descripcion ="Insert� la Unidad Fisica ".$as_coduniadm." Asociado a la Empresa ".$this->ls_codemp;
	...
	function uf_saf_update_unidadfisica($as_coduniadm,$as_denuniadm,$aa_seguridad)
	{
		...
		// Fecha Creaci�n: 03/04/2006 								Fecha �ltima Modificaci�n : 03/04/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->unidadfisica M�TODO->uf_saf_update_unidadfisica ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualiz� la Unidad Fisica ".$as_coduniadm." Asociado a la Empresa ".$this->ls_codemp;
			$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
...
//----------------------------------------------------------------------------------------------------------------------------------
	function uf_saf_delete_unidadfisica($as_coduniadm,$aa_seguridad)
	{
		...
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creaci�n: 22/11/2007								Fecha �ltima Modificaci�n :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if(!$lb_valido)
		{
			...
			if($rs_data===false)
			{
				$this->io_msg->message("CLASE->unidadfisica M�TODO->uf_saf_delete_unidadfisica ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
				$this->io_sql->rollback();
			}
			else
			{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Elimin� la Unidad Fisica ".$as_coduniadm." Asociado a la Empresa ".$this->ls_codemp;
...
//----------------------------------------------------------------------------------------------------------------------------------
	function uf_saf_selectactivos($as_coduniadm)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->unidadfisica M�TODO->uf_saf_selectactivos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	 ...
	 *
	 * @param string $ls_codubifisresuso Código de ubicación fisica de la ubicación
	 *				física.
