...
class sigesp_saf_c_movimiento
{
	...
	function  uf_saf_insert_movimento($as_codemp,$as_cmpmov,$as_codcau,$ad_feccmp,$as_descmp,$as_codpro,$as_cedbene,
									  ...
									  $as_coduniadm,$as_ubigeo,$as_tiprespri,$as_tipresuso,$as_fecent,$as_tipcmp,$as_numcmp,$as_codubifis)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//                 as_codubicfisica ---> Ubicación Física del Activo
		//	      Returns: Retorna un Booleano
		...
		$ls_sql = " INSERT INTO saf_movimiento (codemp,cmpmov,codcau,feccmp,descmp,cod_pro,ced_bene,codtipdoc,codusureg,estpromov, ".
	              "                             codrespri,codresuso,coduniadm,ubigeoact,tiprespri,tipresuso,fecentact,estcat,tipcmp,numcmp,estmov,codubifis) ".
				  ...
				  "       '".$as_fecent."',".$li_estcat.",'".$as_tipcmp."','".$as_numcmp."','R','".$as_codubifis."')";
	...
	function  uf_saf_insert_dt_movimiento($as_codemp,$as_cmpmov,$as_codcau,$ad_feccmp,$as_codact,$as_ideact,
	                                      $as_desmov,$ai_monact,$as_estsoc,$as_estmov,$aa_seguridad,$as_codubifisresuso)
	{// EN MODIFICACI�N POR JOSE LUIS AGUILERA
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_codemp = $_SESSION["la_empresa"]["codemp"];
		$li_estcat=$this->io_activo->uf_select_valor_config($ls_codemp);
		$ls_sql = " INSERT INTO saf_dt_movimiento (codemp,cmpmov,codcau,feccmp,codact,ideact,desmov,monact,estsoc,estmov,estcat,codubifis) ".
				  " VALUES('".$as_codemp."','".$as_cmpmov."','".$as_codcau."','".$ad_feccmp."','".$as_codact."','".$as_ideact."',".
				  "        '".$as_desmov."','".$ai_monact."','".$as_estsoc."','".$as_estmov."','".$li_estcat."','".$as_codubifisresuso."')";
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//	     Function: uf_saf_update_dt_movimiento
	//         Access: public
	//      Argumento: $as_codemp     //codigo de empresa
	//                 $as_cmpmov     //N� del Comprobante del Movimiento
	//                 $as_codcau     //codigo de causa de movimiento
	...
	//				   $aa_seguridad  //arreglo de registro de seguridad
	//	      Returns: Retorna un Booleano
	//    Description: Funcion que actualiza el detalle del movimiento en saf_dt_movimiento
	//	   Creado Por: TSU. José Luis Aguilera M
	// Fecha Creaci�n: 17/04/2013 								Fecha Ultima Modificacion :
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function  uf_saf_update_dt_movimiento($as_codemp,$as_codact,$ai_cosact,$aa_seguridad)
	{
		//file_put_contents('/tmp/sugau_saf.log',"uf_saf_update_dt_movimiento --", FILE_APPEND);
		//$args = func_get_args();
		//file_put_contents('/tmp/sugau_saf.log',print_r($args, TRUE), FILE_APPEND);
		//file_put_contents('/tmp/sugau_saf.log'," -- uf_saf_update_dt_movimiento\n", FILE_APPEND);
		$lb_valido=false;
		$ls_sql="UPDATE saf_dt_movimiento".
				" SET monact='".$ai_cosact."'".
				" WHERE saf_dt_movimiento.codemp =  '".$as_codemp ."'".
				" AND saf_dt_movimiento.codact =  '".$as_codact ."'";
		$li_row = $this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->activo MÉTODO->uf_saf_update_dt_movimiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		...
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualizo el monto del activo en movimiento ".$as_codact." de la Empresa ".$as_codemp;
	...
	/**
	 * Antiguamente llamada uf_saf_update_dtaincorporacion.
	 */
	function uf_saf_insert_dtaincorporacion($as_codemp,$as_codact,$as_ideact,$as_estact,$ad_fecincact,
																					$as_coduniadm,$as_codrespri,$as_codresuso,$as_codubifisresuso,
																					$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		if($ldt_fecdesact == "") { $ldt_fecdesact="1900-01-01"; }
		$ls_sql = "INSERT INTO saf_dta";
		$ls_sql.= " (codemp,codact,estact,fecincact,fecdesact,coduniadm,codrespri,codres,codubifis)";
		$ls_sql.= " VALUES('".$as_codemp."','".$as_codact."','".$as_estact."',";
		$ls_sql.= "'".$ad_fecincact."','".$ldt_fecdesact."',";
		$ls_sql.= "'".$as_coduniadm."','".$as_codrespri."','".$as_codresuso."','".$as_codubifisresuso."')";
		$li_row = $this->io_sql->execute($ls_sql);

		if($li_row === false)
		{
			$this->io_msg->message("Error en el metodo uf_saf_insert_dtaincorporacion de la clase saf_c_movimiento.".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="Incorporo el Activo ".$as_codact." asociado a la Empresa ".$as_codemp;
			$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
...
//---------------------------------------------------------------------------------------------------------------------------------
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/**
	 * Consulta las causas de movimiento de tipo (I)ncorporación.
	 *
	 * @param string $ls_codcau El código de la causa de movimiento seleccionado
	 *				actualmente.
	 *
	 * @return string $causas_incorporacion Las <option> del <select>.
	 */
	 function uf_select_causa_movimiento_incorporacion($ls_codcau)
	 {
		 $ls_sql = "SELECT * FROM saf_causas";
		 $ls_sql.= " WHERE tipcau= 'I'";
		 $ls_sql.= " AND estcat='1'";
		 $ls_sql.= " ORDER BY codcau";

		 $result_set = $this->io_sql->select($ls_sql);
		 $rs_data = $this->io_sql->obtener_datos($result_set);

		 $codcau_array = $rs_data["codcau"];
		 $longitud = count($codcau_array);
		 $dencau_array = $rs_data["dencau"];

		 $causas_incorporacion = '<option value="0">Seleccione</option>';
		 for($i=1;$i<=$longitud;$i++)
		 {
			 if($codcau_array[$i] == $ls_codcau)
			 {
				 $causas_incorporacion .= '<option value="'.$codcau_array[$i].'" selected>'.$dencau_array[$i].'</option>';
			 }else {
				 $causas_incorporacion .= '<option value="'.$codcau_array[$i].'">'.$dencau_array[$i].'</option>';
			 }
		 }
		 return $causas_incorporacion;
	 }
}//fin de la clase
?>
