...
class sigesp_saf_c_movimientos
{
	...
	//////////////////////////////////////////////////////////////////////////////
		...
		if($li_exec===false)
		{
			$this->io_msg->message("CLASE->movimientos M�TODO->uf_saf_select_movimientos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	 ...
	 */
	function  uf_saf_insert_movimientos($as_codigo,$as_denominacion,$as_explicacion,$as_tipo,$ai_contable,$ai_presupuestaria,$aa_seguridad)
	{
		global $fun;
		global $msg;
		//$lb_valido=true;
    ...
    $this->io_sql->begin_transaction();
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="INSERT";
				$ls_descripcion ="Insert&oacute; la Causa de Movimiento ".$as_codigo;
	 ...
	 */
	function uf_saf_update_movimientos($as_codigo,$as_denominacion,$as_explicacion,$as_radiotipo,$ai_contable,$ai_presupuestaria,$aa_seguridad)
	{
	 //$lb_valido=true;
	 $ls_cadena="";
	 $li_exce=-1;
    ...
    $this->io_sql->begin_transaction();
		...
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="UPDATE";
			$ls_descripcion ="Actualiz&oacute; la Causa de Movimiento ".$as_codigo;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			if($ls_variable[0] == false)
  ...
  return json_encode($lb_valido);
	...
	function uf_saf_delete_movimientos($as_codigo,$aa_seguridad)
	{
		...
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
			$ls_evento="DELETE";
			$ls_descripcion ="Elimin&ócute; la Causa de Movimiento ".$as_codigo;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			if($ls_variable[0] == false)
