<?php
   ...
   function uf_imprimirresultados($as_mes,$as_ano)
   {
		...
		$ls_sql="SELECT SUM(mondepmen) AS monto  ".
				...
				"   AND date_part('year',fecdep) = '".$as_ano."' ".
				"   AND date_part('month',fecdep) = '".$as_mes."' ".
				" GROUP BY date_part('year',fecdep), date_part('month',fecdep) ";
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				$ls_sql="SELECT SUM(saf_depreciacion.mondepmen) AS monto, saf_activo.codestpro1, saf_activo.codestpro2, ".
						...
						"   AND date_part('year',saf_depreciacion.fecdep) = '".$as_ano."' ".
						"   AND date_part('month',saf_depreciacion.fecdep) = '".$as_mes."' ".
